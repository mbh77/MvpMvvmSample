...
namespace MvpMvvm.Common
{
    ...
    public abstract class ParametersBase : IParameters
    {
        ...
        {
            var entry = _entries.Find(x => x.Key == key);
        ...
        {
            var entries = _entries.FindAll(x => x.Key == key).ToList().Select(e => e.Value);
            return (IEnumerable<T>)entries;
        }
